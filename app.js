...
function networthTotalForSnap(snap){
  // Full net worth at a snapshot: calculated cash/UT accounts + manual values
  const cutoff = endOfMonth(snap.month);
  let total = 0;
  DB.accounts.forEach(a=>{ total += accountBalance(a, cutoff); });
  DB.manualAccounts.forEach(m=>{ total += (snap.values||{})[m.name]||0; });
  return total;
}

function networthForYearEnd(year){
  // Year-end net worth: December snapshot or latest available month that year
  const snaps = DB.networth.filter(s=>s.month.slice(0,4)===year).sort((a,b)=>b.month.localeCompare(a.month));
  return snaps.length ? networthTotalForSnap(snaps[0]) : null;
}

function networthMonthlyFilled(year){
  // 12 values Jan–Dec: carry-forward last known snapshot, null for future months
  const today = new Date().toISOString().slice(0,7);
  // Find the latest snapshot at or before the start of this year (to seed carry-forward)
  const prior = DB.networth.filter(s=>s.month < `${year}-01`).sort((a,b)=>b.month.localeCompare(a.month));
  let lastSnap = prior[0] || null;
  return Array.from({length:12}, (_,i)=>{
    const m = `${year}-${String(i+1).padStart(2,'0')}`;
    if(m > today) return null;
    const snap = DB.networth.find(s=>s.month===m);
    if(snap) lastSnap = snap;
    return lastSnap ? networthTotalForSnap(lastSnap) : null;
  });
}
...
function incomeBySourceYear(year){
  const rows = DB.income.filter(r=>r.date.slice(0,4)===year && r.source!=='Account Transfer');
  const map = {};
  let total = 0;
  for(const r of rows){ map[r.source]=(map[r.source]||0)+r.amount; total+=r.amount; }
  return {map, total};
}
function expenseByCategoryYear(year){
  const rows = DB.expenses.filter(r=>r.date.slice(0,4)===year);
  const map = {};
  let total = 0;
  for(const r of rows){ map[r.category]=(map[r.category]||0)+r.amount; total+=r.amount; }
  return {map, total};
...
function availableYears(){
  ...
  DB.income.forEach(r=>ys.add(r.date.slice(0,4)));
  DB.expenses.forEach(r=>ys.add(r.date.slice(0,4)));
...
/* ============ NAVIGATION ============ */
let CUR = {screen:'dashboard', month: new Date().toISOString().slice(0,7), histTab:'all', histSearch:'', histLimit:150, balanceDate: new Date().toISOString().slice(0,10), expandedNW:null, advancedOpen:false, insightsMode:'month', insightsMonth: new Date().toISOString().slice(0,7), insightsYear: String(new Date().getFullYear()), trendYears: null};
...
function render(){
  ...
  else if(CUR.screen==='insights') root.innerHTML = renderInsights();
...
const YEAR_COLORS  = ['#34D8A8','#5B9CF2','#E8B860','#F2725C','#9D7FE8','#5FD0E0','#E08FC0','#C4D85A'];

function monthlyBarChart(values, color, showAmt){
  // Vertical bar chart for 12 months
  const nonNull = values.filter(v=>v!=null);
  const max = nonNull.length ? Math.max(1, ...nonNull) : 1;
  return `<div class="trend-month-chart">
    ${values.map((v,i)=>{
      const pct = v!=null ? Math.max(3, Math.round(v/max*100)) : 0;
      const hasVal = v!=null && v>0;
      return `<div class="trend-month-col">
        <div class="trend-month-bar-wrap">
          <div class="trend-month-bar" style="height:${pct}%; background:${color}; opacity:${v!=null?1:0.15};"></div>
        </div>
        <div class="trend-month-lbl">${MONTH_LABELS[i]}</div>
      </div>`;
    }).join('')}
  </div>`;
}

function multiYearBar(yearData, color){
  // Horizontal bar for each year with growth %
  const max = Math.max(1, ...yearData.map(d=>d.value||0));
  return yearData.map((d,i)=>{
    const pct = d.value!=null ? Math.max(2, Math.round((d.value||0)/max*100)) : 0;
    const prev = yearData[i-1];
    let growthHtml = '';
    if(i>0 && prev && prev.value && d.value!=null){
      const g = Math.round((d.value - prev.value)/Math.abs(prev.value)*100);
      growthHtml = `<span class="trend-growth" style="color:${g>=0?'var(--emerald)':'var(--coral)'}">${g>=0?'▲':'▼'}${Math.abs(g)}%</span>`;
    }
    return `<div class="bar-item">
      <div class="bar-top">
        <span class="name" style="font-family:'JetBrains Mono',monospace; font-weight:700;">${d.year}</span>
        <div style="display:flex; align-items:center; gap:8px;">
          ${growthHtml}
          <span class="val">${d.value!=null ? fmt(d.value) : '—'}</span>
        </div>
      </div>
...
function renderTrendsBody(){
  ...
  if(!CUR.trendYears) CUR.trendYears = new Set(allYears);
  const selected = allYears.filter(y=>CUR.trendYears.has(y));
  const today = new Date().toISOString().slice(0,7);

  const selLabel = selected.length===0 ? 'None selected'
    : selected.length===allYears.length ? `All ${allYears.length} years`
    : selected.length<=3 ? selected.join(', ')
    : `${selected[0]}–${selected[selected.length-1]} (${selected.length} years)`;

  const selectorHtml = `<div class="trend-year-bar">
    <span style="font-size:12px; color:var(--text-dim);">Years</span>
    <button class="trend-year-btn" id="openYearPicker">
      <span>${esc(selLabel)}</span>
      <span style="color:var(--text-faint); font-size:13px;">▾</span>
    </button>
  </div>`;

  if(!selected.length) return selectorHtml + `<div class="empty">Select at least one year</div>`;

  const isSingle = selected.length===1;

  if(isSingle){
    const year = selected[0];
    const incVals = MONTH_LABELS.map((_,i)=>{
      const m = `${year}-${String(i+1).padStart(2,'0')}`;
      return m<=today ? incomeBySource(m).total : null;
    });
    const expVals = MONTH_LABELS.map((_,i)=>{
      const m = `${year}-${String(i+1).padStart(2,'0')}`;
      return m<=today ? expenseByCategory(m).total : null;
    });
    const nwVals = networthMonthlyFilled(year);
    const incTotal = incVals.reduce((s,v)=>s+(v||0),0);
    const expTotal = expVals.reduce((s,v)=>s+(v||0),0);
    const bestIncMonth = incVals.reduce((bi,v,i)=>v!=null&&(bi<0||v>incVals[bi])?i:bi, -1);
    const bestExpMonth = expVals.reduce((bi,v,i)=>v!=null&&(bi<0||v>expVals[bi])?i:bi, -1);
    const latestNW = [...nwVals].reverse().find(v=>v!=null);

    return selectorHtml + `
      <div class="section-label">📈 Income — ${year}
        <span class="mono" style="color:var(--emerald); font-weight:700; float:right;">${fmt(incTotal)}</span>
      </div>
      <div class="card">
        ${monthlyBarChart(incVals, 'var(--emerald)')}
        ${bestIncMonth>=0?`<div style="text-align:center; font-size:11.5px; color:var(--text-dim); margin-top:10px;">Peak: <span style="color:var(--emerald); font-weight:600;">${MONTH_LABELS[bestIncMonth]}</span> · ${fmt(incVals[bestIncMonth])}</div>`:''}
      </div>
      <div class="section-label">📉 Expenses — ${year}
        <span class="mono" style="color:var(--coral); font-weight:700; float:right;">${fmt(expTotal)}</span>
      </div>
      <div class="card">
        ${monthlyBarChart(expVals, 'var(--coral)')}
        ${bestExpMonth>=0?`<div style="text-align:center; font-size:11.5px; color:var(--text-dim); margin-top:10px;">Peak: <span style="color:var(--coral); font-weight:600;">${MONTH_LABELS[bestExpMonth]}</span> · ${fmt(expVals[bestExpMonth])}</div>`:''}
      </div>
      <div class="section-label">💰 Net Worth — ${year}
        ${latestNW!=null?`<span class="mono" style="color:var(--violet); font-weight:700; float:right;">${fmt(latestNW)}</span>`:''}
      </div>
      <div class="card">
        ${monthlyBarChart(nwVals, 'var(--violet)')}
        <div style="text-align:center; font-size:11px; color:var(--text-faint); margin-top:10px;">Flat segments = carry-forward from last snapshot</div>
      </div>`;
  }

  // MULTI-YEAR — annual totals
  const incData2 = selected.map(y=>({year:y, value: incomeBySourceYear(y).total}));
  const expData  = selected.map(y=>({year:y, value: expenseByCategoryYear(y).total}));
  const nwData   = selected.map(y=>({year:y, value: networthForYearEnd(y)}));

  return selectorHtml + `
    <div class="section-label">📈 Income by year</div>
    <div class="card">${multiYearBar(incData2, 'var(--emerald)')}</div>
    <div class="section-label">📉 Expenses by year</div>
    <div class="card">${multiYearBar(expData, 'var(--coral)')}</div>
    <div class="section-label">💰 Net Worth by year</div>
    <div class="card">${multiYearBar(nwData, 'var(--violet)')}
      <div style="text-align:center; font-size:11px; color:var(--text-faint); margin-top:10px;">Using last available snapshot per year</div>
    </div>`;
}

/* ============ INSIGHTS ============ */
function renderInsights(){
  const mode = CUR.insightsMode;
  const tabRow = `<div class="tabs">
    <div class="tab ${mode==='month'?'active':''}" data-insmode="month">Month</div>
    <div class="tab ${mode==='year'?'active':''}" data-insmode="year">Year</div>
    <div class="tab ${mode==='trends'?'active':''}" data-insmode="trends">Trends</div>
  </div>`;

  if(mode==='trends'){
    return `<div class="screen"><div style="height:8px"></div>${tabRow}${renderTrendsBody()}</div>`;
  }

  const isMonth = mode==='month';
  const periodLabel = isMonth ? monthLabel(CUR.insightsMonth) : CUR.insightsYear;
  const {map: incMap, total: incTotal} = isMonth ? incomeBySource(CUR.insightsMonth) : incomeBySourceYear(CUR.insightsYear);
  const {map: expMap, total: expTotal} = isMonth ? expenseByCategory(CUR.insightsMonth) : expenseByCategoryYear(CUR.insightsYear);
  const incSorted = Object.entries(incMap).sort((a,b)=>b[1]-a[1]);
  const expSorted = Object.entries(expMap).sort((a,b)=>b[1]-a[1]);
  const net = incTotal - expTotal;
  const savingsRate = incTotal>0 ? Math.round(net/incTotal*100) : null;
  const years = availableYears();
  const atTodayMonth = CUR.insightsMonth >= new Date().toISOString().slice(0,7);
  ...
  return `<div class="screen">
    ...
    ${isMonth ? `
      <div class="bal-datenav" style="margin-bottom:18px;">
        <button class="iconbtn" id="insPrevMonth" aria-label="Previous month">‹</button>
        <input type="month" id="insMonthInput" value="${CUR.insightsMonth}" max="${new Date().toISOString().slice(0,7)}">
        <button class="iconbtn" id="insNextMonth" aria-label="Next month" ${atTodayMonth?'disabled':''}>›</button>
      </div>
    ` : `
      <div class="bal-datenav" style="margin-bottom:18px;">
        <button class="iconbtn" id="insPrevYear" aria-label="Previous year">‹</button>
        <select id="insYearInput" style="flex:1; background:var(--card); border:1px solid var(--border); border-radius:10px; padding:8px 10px; color:var(--text); font-family:'JetBrains Mono',monospace; font-size:13px; text-align:center;">
          ${years.map(y=>`<option value="${y}" ${y===CUR.insightsYear?'selected':''}>${y}</option>`).join('')}
        </select>
        <button class="iconbtn" id="insNextYear" aria-label="Next year" ${CUR.insightsYear>=String(new Date().getFullYear())?'disabled':''}>›</button>
      </div>
    `}

    <div class="card tight">
      <div class="hero-row" style="margin-top:0;">
        <div class="hero-chip"><div class="k"><span class="dot" style="background:var(--emerald)"></span>Income</div><div class="v">${fmtShort(incTotal)}</div></div>
        <div class="hero-chip"><div class="k"><span class="dot" style="background:var(--coral)"></span>Expenses</div><div class="v">${fmtShort(expTotal)}</div></div>
        <div class="hero-chip"><div class="k"><span class="dot" style="background:${net>=0?'var(--emerald)':'var(--coral)'}"></span>Net</div><div class="v">${fmtShort(net)}</div></div>
      </div>
      ${savingsRate!==null ? `<div style="text-align:center; margin-top:14px; font-size:12px; color:var(--text-dim);">Savings rate this ${isMonth?'month':'year'}: <span class="mono" style="color:${savingsRate>=0?'var(--emerald)':'var(--coral)'}; font-weight:700;">${savingsRate}%</span></div>` : ''}
    </div>

    <div class="card" style="margin-top:14px;">
      <div class="card-head"><div class="title">📈 Income — ${esc(periodLabel)}</div><div class="total" style="color:var(--emerald)">${fmt(incTotal)}</div></div>
      ${incSorted.length? incSorted.map(([k,v])=>{
        const pct = incTotal? Math.round(v/incTotal*100):0;
        const c = colorFor(k, DB.incomeSources);
        return `<div class="bar-item"><div class="bar-top"><span class="name">${esc(k)}</span><span class="val">${fmt(v)}</span></div>
          <div class="bar-track"><div class="bar-fill" style="width:${pct}%; background:${c}"></div></div></div>`;
      }).join('') : `<div class="empty">No income recorded for ${esc(periodLabel)}</div>`}
    </div>

    <div class="card">
      <div class="card-head"><div class="title">📉 Expenses — ${esc(periodLabel)}</div><div class="total" style="color:var(--coral)">${fmt(expTotal)}</div></div>
      ${expSorted.length? expSorted.map(([k,v])=>{
        const pct = expTotal? Math.round(v/expTotal*100):0;
        const c = colorFor(k, DB.expenseCategories);
        return `<div class="bar-item"><div class="bar-top"><span class="name">${esc(k)}</span><span class="val">${fmt(v)}</span></div>
          <div class="bar-track"><div class="bar-fill" style="width:${pct}%; background:${c}"></div></div></div>`;
      }).join('') : `<div class="empty">No expenses recorded for ${esc(periodLabel)}</div>`}
    </div>
  </div>`;
}
  ...
  return `<div class="screen">
    ...
    <div class="card tight">
      <div class="settings-row">
        <div><div class="label">${esc(syncStatus.email||'')}</div><div class="hint">Signed in · data synced to Firebase</div></div>
      </div>
      <button class="btn-secondary" id="signOutBtn" style="color:var(--coral); margin-top:10px;">Sign out</button>
    </div>
    ...
    <div class="card tight">
      <div class="row-list">
        ${DB.incomeSources.map(s=>`<div class="settings-row settings-row-tap" data-edit-incsrc="${esc(s)}"><div class="label">${esc(s)}</div><span class="nw-chevron">›</span></div>`).join('')}
      </div>
    </div>
    <button class="btn-secondary" id="addIncSrcBtn">+ Add income source</button>

    <div class="section-label">Expense categories</div>
    <div class="card tight">
      <div class="row-list">
        ${DB.expenseCategories.map(c=>`<div class="settings-row settings-row-tap" data-edit-expcat="${esc(c)}"><div class="label">${esc(c)}</div><span class="nw-chevron">›</span></div>`).join('')}
      </div>
    </div>
    <button class="btn-secondary" id="addExpCatBtn">+ Add expense category</button>
    ...
    <div style="text-align:center; color:var(--text-faint); font-size:11.5px; margin-top:18px;">FinTrack · data synced to Firebase · ${esc(syncStatus.email||'')}</div>
  </div>`;
}
...
function bindScreenEvents(){
  ...
  if(CUR.screen==='insights'){
    document.querySelectorAll('[data-insmode]').forEach(t=> t.onclick = ()=>{ CUR.insightsMode = t.dataset.insmode; render(); });
    document.querySelectorAll('[data-trendyear]').forEach(chip=>{
      chip.onclick = ()=>{
        const y = chip.dataset.trendyear;
        if(CUR.trendYears.has(y)) CUR.trendYears.delete(y); else CUR.trendYears.add(y);
        render();
      };
    });
    const yp = document.getElementById('openYearPicker');
    if(yp) yp.onclick = openTrendYearPickerSheet;
    const mi = document.getElementById('insMonthInput');
    if(mi) mi.onchange = e=>{ CUR.insightsMonth = e.target.value || CUR.insightsMonth; render(); };
    const pm = document.getElementById('insPrevMonth');
    if(pm) pm.onclick = ()=>{
      const d = new Date(CUR.insightsMonth+'-01T00:00:00'); d.setMonth(d.getMonth()-1);
      CUR.insightsMonth = d.toISOString().slice(0,7); render();
    };
    const nm = document.getElementById('insNextMonth');
    if(nm) nm.onclick = ()=>{
      const d = new Date(CUR.insightsMonth+'-01T00:00:00'); d.setMonth(d.getMonth()+1);
      const next = d.toISOString().slice(0,7);
      CUR.insightsMonth = next > todayStr().slice(0,7) ? todayStr().slice(0,7) : next;
      render();
    };
    const yi = document.getElementById('insYearInput');
    if(yi) yi.onchange = e=>{ CUR.insightsYear = e.target.value; render(); };
    const py = document.getElementById('insPrevYear');
    if(py) py.onclick = ()=>{ CUR.insightsYear = String(parseInt(CUR.insightsYear,10)-1); render(); };
    const ny = document.getElementById('insNextYear');
    if(ny) ny.onclick = ()=>{
      const next = parseInt(CUR.insightsYear,10)+1;
      CUR.insightsYear = String(Math.min(next, new Date().getFullYear()));
      render();
    };
  }
  ...
  if(CUR.screen==='settings'){
    ...
    document.querySelectorAll('[data-edit-incsrc]').forEach(el=> el.onclick = ()=>openEditCategorySheet('income', el.dataset.editIncsrc));
    document.querySelectorAll('[data-edit-expcat]').forEach(el=> el.onclick = ()=>openEditCategorySheet('expense', el.dataset.editExpcat));
    document.getElementById('addIncSrcBtn').onclick = ()=>openAddCategorySheet('income');
    document.getElementById('addExpCatBtn').onclick = ()=>openAddCategorySheet('expense');
    ...
    document.getElementById('importFile').onchange = (e)=>{
      ...
      reader.onload = (ev)=>{
        ...
        openConfirmSheet('Import this backup?','This replaces your current data with the contents of the file. Changes will also sync to the cloud. This cannot be undone.','Import & replace', ()=>{
    ...
    if(resetBtn) resetBtn.onclick = ()=>{
      openConfirmSheet('Reset all data?','This permanently deletes your local copy. Your cloud data in Firebase is unaffected unless you also push the empty state. Export a backup first if unsure.','Reset local data', ()=>{
        closeSheet();
        localStorage.removeItem(LS_KEY); DB = defaultDB(); render(); toast('Local data cleared');
      });
    };
    const signOutBtn = document.getElementById('signOutBtn');
    if(signOutBtn) signOutBtn.onclick = ()=>{
      openConfirmSheet('Sign out?','You can sign back in at any time. Your data stays safe in Firebase.','Sign out', ()=>{
        Sync.signOut(); closeSheet();
      });
    };
  }
}
  ...
  openSheet(`
    ...
    ${['unittrust','other','liability'].map(type=>{
      ...
      return `<div class="section-label" style="margin-top:16px">${heading}</div>` + items.map(m=>
        `<div class="field"><label>${esc(m.name)}</label><input type="text" class="amt-input" data-acc="${esc(m.name)}" value="${vals[m.name]!=null?vals[m.name]:''}" placeholder="${m.type==='liability'?'e.g. -11000':'0.00'}"></div>`
      ).join('');
    }).join('')}
...
function openTrendYearPickerSheet(){
  const allYears = availableYears();
  openSheet(`
    <div class="sheet-handle"></div>
    <h2>Select years</h2>
    <div class="sub">Tap years to include in the trend view. Select 1 year to see monthly breakdown.</div>
    <div style="display:flex; gap:10px; margin-bottom:14px;">
      <button class="btn-secondary" id="ypSelectAll" style="flex:1; padding:10px;">All</button>
      <button class="btn-secondary" id="ypClearAll" style="flex:1; padding:10px;">None</button>
    </div>
    <div class="card tight" style="margin-bottom:0">
      ${allYears.map(y=>`
        <div class="settings-row yp-row" data-yr="${y}" style="cursor:pointer;">
          <div class="label" style="font-family:'JetBrains Mono',monospace; font-weight:700;">${y}</div>
          <div class="yp-check ${CUR.trendYears.has(y)?'checked':''}"></div>
        </div>`).join('')}
    </div>
    <button class="btn-primary" id="ypApply" style="margin-top:14px;">Apply</button>
  `);
  // work on a local copy so Apply is the confirmation
  const draft = new Set(CUR.trendYears);
  function refresh(){
    document.querySelectorAll('.yp-row').forEach(row=>{
      row.querySelector('.yp-check').classList.toggle('checked', draft.has(row.dataset.yr));
    });
  }
  document.querySelectorAll('.yp-row').forEach(row=>{
    row.onclick = ()=>{ const y=row.dataset.yr; draft.has(y)?draft.delete(y):draft.add(y); refresh(); };
  });
  document.getElementById('ypSelectAll').onclick = ()=>{ allYears.forEach(y=>draft.add(y)); refresh(); };
  document.getElementById('ypClearAll').onclick = ()=>{ draft.clear(); refresh(); };
  document.getElementById('ypApply').onclick = ()=>{ CUR.trendYears = draft; closeSheet(); render(); };
}

function openEditCategorySheet(type, name){
  const isIncome = type==='income';
  const list = isIncome ? DB.incomeSources : DB.expenseCategories;
  const recordCount = isIncome
    ? DB.income.filter(r=>r.source===name).length
    : DB.expenses.filter(r=>r.category===name).length;
  const typeLabel = isIncome ? 'income source' : 'expense category';

  openSheet(`
    <div class="sheet-handle"></div>
    <h2>Edit ${esc(typeLabel)}</h2>
    <div class="sub">${recordCount} existing record(s) use "${esc(name)}". Renaming will update all of them.</div>
    <div class="field"><label>Name</label><input id="catName" value="${esc(name)}"></div>
    <button class="btn-primary" id="saveCat">Save changes</button>
    <button class="btn-secondary" id="deleteCat" style="color:var(--coral)">Delete ${esc(typeLabel)}</button>
  `);
  document.getElementById('saveCat').onclick = ()=>{
    const newName = document.getElementById('catName').value.trim();
    if(!newName){ toast('Enter a name'); return; }
    if(newName!==name && list.includes(newName)){ toast('Already exists'); return; }
    const idx = list.indexOf(name);
    if(idx>=0) list[idx] = newName;
    if(isIncome) DB.income.forEach(r=>{ if(r.source===name) r.source=newName; });
    else DB.expenses.forEach(r=>{ if(r.category===name) r.category=newName; });
  ...
  document.getElementById('deleteCat').onclick = ()=>{
    openConfirmSheet(
      `Delete "${name}"?`,
      recordCount
        ? `${recordCount} record(s) use this ${typeLabel}. They'll remain in your history under the same name but it won't be selectable for new entries.`
        : `Removes "${name}" from the list.`,
      'Delete',
      ()=>{
        if(isIncome) DB.incomeSources = DB.incomeSources.filter(s=>s!==name);
        else DB.expenseCategories = DB.expenseCategories.filter(c=>c!==name);
        persist(); closeSheet(); render(); toast('Deleted');
      }
    );
  };
...
function openAddCategorySheet(type){
  const isIncome = type==='income';
  const typeLabel = isIncome ? 'income source' : 'expense category';
  openSheet(`
    <div class="sheet-handle"></div>
    <h2>Add ${esc(typeLabel)}</h2>
    <div class="sub">It will immediately appear as an option when adding a new ${isIncome?'income entry':'expense'}.</div>
    <div class="field"><label>Name</label><input id="catName" placeholder="${isIncome?'e.g. Freelance, Rental':'e.g. Travel, Childcare'}"></div>
    <button class="btn-primary" id="saveCat">Add</button>
  `);
  document.getElementById('saveCat').onclick = ()=>{
    const name = document.getElementById('catName').value.trim();
    if(!name){ toast('Enter a name'); return; }
    if(isIncome){
      if(DB.incomeSources.includes(name)){ toast('Already exists'); return; }
      DB.incomeSources.push(name);
    } else {
      if(DB.expenseCategories.includes(name)){ toast('Already exists'); return; }
...
function showAppScreen(){ document.getElementById('authScreen').style.display='none'; document.getElementById('app').style.display='block'; }

function initAuthScreen(){
  let isSignUp = false;
  const emailEl   = document.getElementById('authEmail');
  const passEl    = document.getElementById('authPass');
  const passConEl = document.getElementById('authPassConfirm');
  const confirmW  = document.getElementById('authConfirmWrap');
  const submitBtn = document.getElementById('authSubmit');
  const toggleBtn = document.getElementById('authToggle');
  const toggleLbl = document.getElementById('authToggleLabel');
  ...
  function setMode(su){
    isSignUp = su;
    submitBtn.textContent  = su ? 'Create account' : 'Log in';
    toggleBtn.textContent  = su ? 'Log in' : 'Sign up';
    toggleLbl.textContent  = su ? 'Already have an account?' : "Don't have an account?";
    confirmW.style.display = su ? '' : 'none';
    forgotBtn.style.display = su ? 'none' : 'block';
    errorEl.style.display  = 'none';
  }
  function setLoading(on){ submitBtn.disabled = on; submitBtn.textContent = on ? 'Please wait…' : (isSignUp ? 'Create account' : 'Log in'); }
  function showError(msg){ errorEl.textContent = msg; errorEl.style.display = 'block'; }

  toggleBtn.onclick = ()=> setMode(!isSignUp);
  eyeBtn.onclick = ()=>{ passEl.type = passEl.type==='password' ? 'text' : 'password'; };

  forgotBtn.onclick = ()=>{
    const email = emailEl.value.trim();
    if(!email){ showError('Enter your email address first'); return; }
    Sync.sendPasswordReset(email).then(()=>{ showError('Password reset email sent — check your inbox.'); errorEl.style.color='var(--emerald)'; }).catch(e=>showError(e.message));
  };
  ...
  [emailEl, passEl, passConEl].forEach(el=> el.addEventListener('keydown', e=>{ if(e.key==='Enter') submitBtn.click(); }));
}
...
function init(){
  ...
  Sync.on('authChange', user=>{
    ...
    // First time signing in (no cloud data yet) vs returning user
    Sync.fetchCloudOnce().then(result=>{
      if(result && result.parsed){
        // Cloud has data — use it (most recent is always authoritative)
        DB = result.parsed;
        saveDB(DB);
      } else {
        // New account — push whatever is in local storage (usually empty seed)
        Sync.pushNow(DB);
      }
      showAppScreen();
      render();
    }).catch(()=>{
      // Fetch failed (e.g. offline) — show whatever we have locally
      showAppScreen();
      render();
    });
  });
  ...
  Sync.on('remoteUpdate', (parsed)=>{
    // Never overwrite local DB while a local change is still queued or in-flight.
    // hasPendingLocal() means we wrote to localStorage but Firebase hasn't
    // confirmed receipt yet — applying the (older) cloud version now would
    // silently erase the user's new entry.
    if(Sync.hasPendingLocal()) return;
    const unchanged = JSON.stringify(parsed) === JSON.stringify(DB);
    if(unchanged) return;
    DB = parsed;
    saveDB(DB);
    render();
  ...
  // Flush any pending local push the moment the app is backgrounded or closed.
  // On Android, JS timers are killed within milliseconds of the app being swiped
  // away, so the 2-second debounce would never fire. visibilitychange and
  // pagehide both fire synchronously before that kill happens, giving us a
  // brief window to push immediately.
  document.addEventListener('visibilitychange', ()=>{
    if(document.visibilityState === 'hidden') Sync.flushPendingPush();
  });
  window.addEventListener('pagehide', ()=> Sync.flushPendingPush());
}
