...
/* ============ NAVIGATION ============ */
let CUR = {screen:'dashboard', month: new Date().toISOString().slice(0,7), histTab:'all', histSearch:'', histLimit:150, balanceDate: new Date().toISOString().slice(0,10), expandedNW:null, advancedOpen:false, insightsMode:'month', insightsMonth: new Date().toISOString().slice(0,7), insightsYear: String(new Date().getFullYear())};
...
function renderInsights(){
  ...
  return `<div class="screen">
    ...
    <div class="tabs">
      ...
      <div class="tab ${!isMonth?'active':''}" data-insmode="year">Year</div>
    </div>
  ...
  openSheet(`
    ...
    ${['unittrust','other','liability'].map(type=>{
      ...
      return `<div class="section-label" style="margin-top:16px">${heading}</div>` + items.map(m=>
        `<div class="field"><label>${esc(m.name)}</label><input type="text" inputmode="decimal" class="amt-input" data-acc="${esc(m.name)}" value="${vals[m.name]!=null?vals[m.name]:''}" placeholder="${m.type==='liability'?'e.g. -11000':'0.00'}"></div>`
      ).join('');
    }).join('')}
