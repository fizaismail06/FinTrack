...
/* ============ NAVIGATION ============ */
let CUR = {screen:'dashboard', month: new Date().toISOString().slice(0,7), histTab:'all', histSearch:'', histLimit:150, balanceDate: new Date().toISOString().slice(0,10), expandedNW:null};
...
function renderSettings(){
  return `<div class="screen">
    ...
    <div class="card tight">
      <div class="row-list">
        ${DB.accounts.map(a=>`<div class="settings-row"><div><div class="label">${esc(a.name)}</div><div class="hint">${a.type} · opening RM ${a.opening||0}</div></div></div>`).join('')}
      </div>
    </div>
    ...
    <div class="card tight">
      <div class="row-list">
        ${DB.manualAccounts.map(a=>`<div class="settings-row"><div><div class="label">${esc(a.name)}</div><div class="hint">${a.type}</div></div></div>`).join('')}
      </div>
    </div>
    ...
    <button class="btn-secondary" id="resetBtn" style="color:var(--coral)">⚠ Reset all data</button>
...
function bindScreenEvents(){
  ...
  if(CUR.screen==='history'){
    ...
    if(s){ s.oninput = e=>{ CUR.histSearch = e.target.value; CUR.histLimit=150; render(); }; s.focus(); s.selectionStart=s.value.length; }
  ...
  if(CUR.screen==='settings'){
    ...
    document.getElementById('resetBtn').onclick = ()=>{
      openConfirmSheet('Reset all data?','This permanently deletes everything stored on this device, including your imported history. This cannot be undone.','Reset everything', ()=>{
        localStorage.removeItem(LS_KEY); DB = defaultDB(); persist(); closeSheet(); render(); toast('Data reset');
      });
    };
  }
}
