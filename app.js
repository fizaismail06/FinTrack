...
function renderTrendsBody(){
  ...
  const chipsHtml = `<div class="chip-row" style="flex-wrap:wrap; gap:8px; margin-bottom:20px;">
    ${allYears.map(y=>`<div class="chip ${CUR.trendYears.has(y)?'sel':''}" data-trendyear="${y}">${y}</div>`).join('')}
  </div>`;

  if(!selected.length) return chipsHtml + `<div class="empty">Select at least one year</div>`;
