...
const Sync = (function(){
  ...
  let pendingSnapshot = null; // the frozen DB copy waiting to be pushed
  ...
  function schedulePush(dbObj){
    ...
    // Deep-clone at the moment of the save so any subsequent mutations to DB
    // (e.g. another entry being added within the 2-second window, or a
    // remoteUpdate arriving) don't silently change what gets pushed to Firebase.
    pendingSnapshot = JSON.parse(JSON.stringify(dbObj));
    pushTimer = setTimeout(()=>pushNow(pendingSnapshot), 2000);
  }

  function pushNow(dbObj){
    if(!auth || !auth.currentUser) return Promise.resolve();
    if(!dbObj) return Promise.resolve(); // already flushed via flushPendingPush
    hasPendingLocalPush = true;
    pendingSnapshot = dbObj;
  ...
  function flushPendingPush(){
    if(hasPendingLocalPush && pendingSnapshot){
      clearTimeout(pushTimer);
      pushNow(pendingSnapshot);
    }
  }

  return {
    init, flushPendingPush,
    signUp, signIn, signOut, sendPasswordReset,
    schedulePush, pushNow, fetchCloudOnce,
    hasPendingLocal: ()=> hasPendingLocalPush,
